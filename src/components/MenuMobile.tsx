...
export const MenuMobile: React.FC<MenuProps> = ({ className }) => {
    ...
    return (
        <>
            <div className={`${className}--menu-hamburger`}>
            {<GoogleIcon className={display ? ` rotate ${className}` : `${className}`} icon={hambrger} />}
            <Link className={`${className}--menu-hamburger__button`} to="/oMnie">O mnie</Link>
                <Link to = "/produkty"
